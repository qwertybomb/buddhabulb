...
static void avx512_xorshift128plus_init(uint64_t key1, uint64_t key2,
 ...
 */
static __m512i avx512_xorshift128plus(avx512_xorshift128plus_key_t *key) {
 ...
 */
static void avx_xorshift128plus_jump(avx_xorshift128plus_key_t * key) {
	uint64_t S0[4];
	uint64_t S1[4];
...
static __m256i avx_randombound_epu32(__m256i randomvals, __m256i upperbound) {
	...
	return _mm256_blend_epi32(evenparts, oddparts, /* 0b10101010 */ 85);
}

static void avx_xorshift128plus_shuffle32(avx_xorshift128plus_key_t *key,
		uint32_t *storage, uint32_t size) {
	avx_xorshift128plus_shuffle32_partial(key, storage, size, 1);
}

static void avx_xorshift128plus_shuffle32_partial(avx_xorshift128plus_key_t *key,
